...
import { emailsArraySchema } from '../components/InviteMembersModal'
 ...
 *
 * @param emailsArray a list of emails to show initially in the UI.
 ...
 */
export function usePills({ emailsArray = [] }: { emailsArray: string[] }) {
  const [pills, setPills] = useState<Pill[]>(
    emailsArray.map(e => ({ id: Date.now().toString(), email: e, shortenedEmail: truncateMaxedOutText(e) })),
  ...
  const getError = (email: string) => {
    if (!emailsArraySchema.safeParse([email]).success) {
  ...
  const removePill = useCallback(
    (pill: Pill, fieldOnChange: (value: string[]) => void) => {
      setPills(prevState => {
        ...
        const newState = prevState.filter(p => p.id !== pill.id)
